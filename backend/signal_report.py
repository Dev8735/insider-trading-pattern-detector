...
def save_signals_csv(signals: list[dict], filename: str = "signal_report.csv"):
    ...
    for s in signals:
        rows.append({
            ...
            "Quality_Score_Pct" : round(s["quality_score"] * 100, 1),
            "RF_Signal"         : s["rf_signal"],
            "Open"              : s["open"],
            "High"              : s["high"],
            "Low"               : s["low"],
            "Close"             : s["close"],
            "Volume"            : s["volume"],
            "Candle_Direction"  : "GREEN" if s["close"] > s["open"] else "RED",
            "Body_Pct"          : round(s["body_pct"], 3),
            "Upper_Shadow_Pct"  : round(s["upper_shadow_pct"], 3),
            "Lower_Shadow_Pct"  : round(s["lower_shadow_pct"], 3),
            "Candle_Score"      : round(s["candle_score"], 3),
            "Volume_Surge_X"    : round(s["vol_surge"], 2),
            "Volume_Assessment" : ("VERY STRONG" if s["vol_surge"] >= 3 else "STRONG" if s["vol_surge"] >= 2 else "MODERATE"),
            "AVR_Baseline_EMA"  : round(s["baseline_avr"], 4),
            "CAR_10_Pct"        : round(s["car_10_pct"], 4),
            "Return_Z_Score"    : round(s["return_z"], 4),
            "Entry_Price"       : s["close"],
            "Stop_Loss"         : s["stop_loss"],
            "Target_1_15pct"    : s["target_1"],
            "Target_2_30pct"    : s["target_2"],
            "Risk_Reward_T1"    : round(abs(s["target_1"] - s["close"]) / max(abs(s["stop_loss"] - s["close"]), 0.01), 2),
            "Evidence_Count"    : len(s["evidence_points"]),
            "Evidence_Points"   : " | ".join(s["evidence_points"]),
            "Hist_Signals_Count": s["historical_count"],
            "Hist_Win_Rate_Pct" : s["win_rate_pct"],
            "Hist_Avg_6M_Return": s["avg_hist_return"],
        })
